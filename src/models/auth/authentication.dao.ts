...
import TenantExistsExecption from "../../exceptions/TenantExistsExecption";
...
class AuthenticationDao implements Dao {
  ...
  public login = async (loginData: UserLoginDto, userAgent: object): Promise<object | Error> => {
    ...
    const tenantRepository: Repository<Tenant> = getConnection().getRepository(Tenant);

    const tenant: Tenant = await tenantRepository.findOne({id: loginData.tenant});
    if (tenant) {
      const user: User = await userRepository.findOne({ email: loginData.email, tenants: tenant}, { relations: ["roles", "tenants" ]});
      if (user) {
      ...
      throw new TenantExistsExecption();
    }
  }
    ...
    if (
      ...
      try {
        const tenant = tenantRepository.create({
          id: userData.tenantName,
          subsType: "free"
        });
        const newTenant: Tenant = await tenantRepository.save(tenant);
        
        const hashedPassword = await hashPassword(userData.password);
        const guestRole = roleRepository.create({id: "tenant"});
        ...
        event.emit(ActivityType.CREATE, {
          ...
          object: {...newTenant, type: ObjectType.Tenant},
          resource: this.resource,
          timestamp: ended,
          took: ended - started,
